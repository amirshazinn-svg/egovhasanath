...
export default function CCEWorksPage() {
  ...
  const [works, setWorks] = useState(mockCCEWorks);
  ...
  const loadData = async () => {
    const [subjectsData, worksData] = await Promise.all([
      getSubjects(),
      getCCEWorks()
    ]);
...
function WorkCard({ work, onClick }: { work: typeof mockCCEWorks[0]; onClick: () => void }) {
  return (
