      ...
      <main className="p-4 pb-24 space-y-6">
        ...
        <Tabs value={leaderboardType} onValueChange={(v) => setLeaderboardType(v as 'students' | 'classes')}>
          ...
          <TabsContent value="students" className="mt-4 space-y-4">
            ...
            <div className="space-y-2">
              {studentData.map((entry) => (
                <StudentLeaderboardCard key={entry.studentId} entry={entry} />
              ))}
            </div>
          </TabsContent>
          ...
          <TabsContent value="classes" className="mt-4 space-y-4">
            ...
            <div className="space-y-2">
              {classData.map((entry) => (
                <ClassLeaderboardCard key={entry.classId} entry={entry} />
              ))}
            </div>
          </TabsContent>
        </Tabs>
...
function StudentLeaderboardCard({ entry }: { entry: LeaderboardEntry }) {
    ...
    <Card className="overflow-hidden">
      <CardContent className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium truncate">{entry.className}</p>
